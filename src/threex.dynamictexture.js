...
// reuse canvas
var canvasDt	= document.createElement( 'canvas' );
/**
...
THREEx.DynamicTexture	= function(width, height){
	...
	canvasDt.width	= width;
	canvasDt.height	= height;
	//this.canvas	= canvasDt;

	var context	= canvasDt.getContext( '2d' );
	this.context	= context;

	var texture	= new THREE.Texture(canvasDt);
...
THREEx.DynamicTexture.prototype.clear = function(fillStyle){
	...
	if( fillStyle !== undefined ){
		...
		this.context.fillRect(0,0,canvasDt.canvas.width, this.canvas.height)
	}else{
		this.context.clearRect(0,0,canvasDt.canvas.width, this.canvas.height)
	}
...
THREEx.DynamicTexture.prototype.drawText = function(text, x, y, fillStyle, contextFont){
	...
	if( x === undefined || x === null ){
		...
		x = (canvasDt.width - textSize.width) / 2;
	}
...
THREEx.DynamicTexture.prototype.drawTextCooked = function(options){
	...
	var params	= {
		...
		fillStyle	: options.fillStyle !== undefined ? options.fillStyle : '#1c75bc',
		font		: options.font !== undefined ? options.font : "bold "+(0.2*512)+"px Arial"
	};
	// sanity check
	console.assert((typeof(text) === 'string') || (typeof(text) === 'number'));

	context.save()
	context.fillStyle	= params.fillStyle;
	context.font		= params.font;

	var y	= (params.lineHeight + params.margin)*canvasDt.height;
	while(text.length > 0 ){
		...
		if( params.align === 'left' ){
			var x	= params.margin*canvasDt.width
		}else if( params.align === 'right' ){
			var x	= (1-params.margin)*canvasDt.width - textSize.width
		}else if( params.align === 'center' ){
			var x = (canvasDt.width - textSize.width) / 2;
		...
		// goto the next line
		y	+= params.lineHeight*canvasDt.height
	}
	context.restore()
	...
	function computeMaxTextLength(text){
		var maxText	= ''
		var maxWidth	= (1-params.margin*2)*canvasDt.width
